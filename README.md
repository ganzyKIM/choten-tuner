...
- **튜너 모드**: 마이크로 소리를 듣고 가장 가까운 음이름(C, C#, … B)과
  센트(cent) 단위 편차를 가로 박스형 미터로 표시. 중앙이 정확, 왼쪽이 낮음(♭),
  오른쪽이 높음(♯). YIN 피치 검출 + 포물선 보간, 미디언 스무딩.
- **악기 프리셋**: 범용 / 기타 / 베이스 / 바이올린 / 해금 / 샤미센 / 피아노.
  악기 음역 밖의 잡음과 옥타브 오검출을 걸러냅니다.
- **마이크 보정**: 측정이 일정하게 높거나 낮게 나오는 환경에서 그 차이를
  센트 단위로 상쇄합니다. 정확한 음을 들려주면서 버튼 한 번으로 자동 보정 가능.
...
- **마스코트**: 화면 아래 캐릭터 상반신이 상황에 맞는 표정으로 바뀌고, 대사창(도스식
  타자기 효과)으로 말을 겁니다. 터치하거나 변신할 때, 음이 딱 맞을 때 별이 뾰로롱
  흩어졌다가 몇 초에 걸쳐 사라집니다.
- **설정**: A4 기준 주파수(435~445Hz), 악기 프리셋, 마이크 보정(±30센트).
...
DSP 코어(음정 수학, YIN 검출기)는 합성 파형 기반 JVM 유닛테스트로 검증합니다.

```
gradlew.bat :app:testDebugUnitTest
```

`YinAccuracyTest`는 C2~E6 전 구간에서 검출 오차를 **센트 단위로** 측정하고
1센트를 넘으면 실패합니다. 현재 실측 최악값은 C6에서 +0.47센트, 나머지 구간은
0.05센트 미만입니다. 즉 실사용에서 몇 센트씩 어긋나 보인다면 원인은 알고리즘이
아니라 마이크·스피커·실내 음향 경로이며, **마이크 보정** 설정으로 상쇄합니다.
