# 초텐 튜너 (Choten Tuner) ♪

도스/Win95 감성 + 핑크핑크 귀염뽀짝 레트로 테마의 안드로이드 조율기(튜너) 앱.
초텐짱 프로젝트(choten-translate)의 디자인 언어와 캐릭터를 그대로 이어받았습니다.

## 기능

- **튜너 모드**: 마이크로 소리를 듣고 가장 가까운 음이름(C, C#, … B)과
  센트(cent) 단위 편차를 가로 박스형 미터로 표시. 중앙이 정확, 왼쪽이 낮음(♭),
  오른쪽이 높음(♯). YIN 피치 검출 + 포물선 보간, 미디언 스무딩.
- **악기 프리셋**: 범용 / 기타 / 베이스 / 바이올린 / 해금 / 샤미센 / 피아노.
  악기 음역 밖의 잡음과 옥타브 오검출을 걸러냅니다.
- **마이크 보정**: 측정이 일정하게 높거나 낮게 나오는 환경에서 그 차이를
  센트 단위로 상쇄합니다. 정확한 음을 들려주면서 버튼 한 번으로 자동 보정 가능.
- **마이크 민감도**: 무음으로 취급할 기준(RMS 게이트)을 1~10단계로 조절합니다.
  숫자가 클수록 작은 소리까지 잡고, 주변이 시끄러우면 낮춰서 잡음을 무시합니다.
  설정 화면에 실시간 입력 레벨 바와 감지 기준선이 함께 표시돼 감으로 고르지
  않아도 됩니다.
- **소리내기 모드**: 원하는 음(옥타브 2~6, 12음)을 골라 기준음을 계속 재생.
  클릭 없는 사인파, 음 전환 시 부드러운 포르타멘토.
- **변신 버튼**: 라이트(초텐짱) ↔ 다크(아메) 테마 전환. 팔레트·캐릭터·말투가
  통째로 바뀝니다. 설정은 저장됨.
- **마스코트**: 화면 아래 캐릭터 상반신이 상황에 맞는 표정으로 바뀌고, 대사창(도스식
  타자기 효과)으로 말을 겁니다. 터치하거나 변신할 때 별이 뾰로롱 흩어졌다가
  몇 초에 걸쳐 사라집니다.
- **설정**: A4 기준 주파수(435~445Hz), 악기 프리셋, 마이크 보정(±30센트),
  마이크 민감도(1~10단계).

### 상황별 표정 매핑

캐릭터 원본은 `txtgame/public/char`의 픽셀아트 5종(폼별)을 상반신으로 크롭해 사용합니다.

| 상황 | Sprite | 초텐짱 | 아메 |
|---|---|---|---|
| 튜닝 중 (소리 감지 중) / 평음(C·F·G) | NEUTRAL | default | default |
| 장음정(D·E·A·B) | JOY | peace | dere |
| 마이크 권한 없음 | SHARP | angry | yandere |
| 튜너 대기 (무음) | FLAT | vape | smoking |
| 단음정·트라이톤(C#·D#·F#·G#·A#) | DARK | vape | drug |

**튜너로 소리가 들어오는 동안에는 표정과 대사가 하나로 고정됩니다.** 음정 편차는
미터가 실시간으로 보여주고, 캐릭터는 조율에 방해되지 않도록 가만히 있습니다.

음색 분류는 C 기준 음정으로 판단합니다 — 완전음정은 평음, 장음정은 밝음,
단음정과 트라이톤은 어두움. 각 상황마다 전용 대사 풀이 따로 있습니다.
상태 전환에는 400ms 안정화 지연이 걸려 경계값에서 깜빡이지 않습니다.

### 터치 반응

캐릭터를 터치하면 폼별로 11~12가지 (포즈 + 대사) 세트 중 하나가 나옵니다.
**직전과 다른 포즈를 강제로 골라서** 연속으로 눌러도 매번 다른 모습이 보입니다.
아메는 dere(부끄러움) / drug(몽롱) / smoking(나른) / yandere(집착) / default(무표정)를
모두 사용합니다.

## 기술 스택

- Kotlin + Jetpack Compose (Material 컴포넌트 없이 커스텀 레트로 위젯)
- minSdk 24 / targetSdk 34, AGP 8.5.2, Gradle 8.7
- 오디오: `AudioRecord`(44.1kHz 모노 캡처) / `AudioTrack`(스트리밍 사인파)
- 피치 검출: YIN (de Cheveigné & Kawahara, 2002) 자체 구현
- 폰트: [Galmuri](https://github.com/quiple/galmuri) (SIL OFL 1.1) — 도스 시절 둥근모꼴 계열 픽셀 폰트

## 빌드

```
gradlew.bat :app:assembleDebug
```

APK 출력: `app/build/outputs/apk/debug/app-debug.apk`

이 PC 특이사항: 보안 프로그램의 SSL 검사 때문에 `gradle.properties`의
`-Djavax.net.ssl.trustStoreType=WINDOWS-ROOT` 플래그가 필요합니다(이미 설정됨).

## 테스트

DSP 코어(음정 수학, YIN 검출기)는 합성 파형 기반 JVM 유닛테스트로 검증합니다.

```
gradlew.bat :app:testDebugUnitTest
```

`YinAccuracyTest`는 C2~E6 전 구간에서 검출 오차를 **센트 단위로** 측정하고
1센트를 넘으면 실패합니다. 현재 실측 최악값은 C6에서 +0.47센트, 나머지 구간은
0.05센트 미만입니다. 즉 실사용에서 몇 센트씩 어긋나 보인다면 원인은 알고리즘이
아니라 마이크·스피커·실내 음향 경로이며, **마이크 보정** 설정으로 상쇄합니다.

## 에뮬레이터로 화면 확인

AVD `choten`(pixel_4, android-34 x86_64, AEHD 가속)이 만들어져 있습니다.

```
C:\Android\Sdk\emulator\emulator.exe -avd choten
```

## 디자인 테마 (초텐짱 / 아메)

| 토큰 | 라이트(초텐짱) | 다크(아메) |
|---|---|---|
| bg | `#f8f0ff` | `#221c2b` |
| pink | `#ffb7e5` | `#d16b8a` |
| purple | `#d1b3ff` | `#8f79c9` |
| border | `#4a4a4a` | `#efe6ff` |
| mint | `#a9f0cb` / `#2fbf77` | `#3e8f6c` / `#5ad695` |

체커보드 배경, 3px 보더 + 하드 섀도 창, 그라데이션 타이틀바(─ □ ✕),
픽셀 폰트, 꾹 눌리는 레트로 버튼이 시그니처.

## 주의

`app/src/main/res/drawable-nodpi/`의 캐릭터 일러스트는 팬 컨텐츠 소재이므로
이 저장소는 비공개(private) 유지를 권장합니다.

made by 간지김 ♥
